...
struct range_node {
  ...
  fctree* fc;  // 这个指针用于指向y数值的数组，记得初始化时要用new
               // fc来确定它的数组大小
  int fc_size = 0;
  ...
  range_node(const station_type& e, range_node* p, fctree* fc1 = NULL,
             range_node* lc1 = NULL, range_node* rc1 = NULL)
      : data(e), parent(p), fc(fc1), lc(lc1), rc(rc1) {}
};
...
int stature(range_node* a) {
  return a ? a->height : -1;
}  // 由于a可能为空，因此需要单独给它一个函数
void rangetree::insertAsRoot(const station_type& e) {
  _root = new range_node(e, NULL);
  size++;
}
...
range_node* common_root() {
  while (0 < road_x1 && 0 < road_x2 &&
    ...
    ;
  return (stack_x1[road_x1] == stack_x2[road_x2]) ? stack_x1[road_x1].c
                                                  : stack_x1[++road_x1].c;
}
// 对每个节点的y值进行2分查找！
...
void travelReport(int x1, int x2, int y1, int y2) {
  ...
  if (now2->data.x >= x1 && now2->data.x <= x2 && now2->data.y >= y1 &&
      now2->data.y <= y2) {
    count++;
...
int main() {
  ...
  while (GetQuery(&x1, &y1, &x2, &y2)) {
    ...
    // 取消注释可以看到路径
    // printf("road%d:= ", num);
    // for (auto i = 0; i < road_x1; i++) { printf("%d(%d) ", stack_x1[i].x,
    // stack_x1[i].y); } printf("\n");
    search_x2(a._root, x1, x2, stack_x2, road_x2, 0, n);
    // printf("road%d:= ", num++);
    // printf("%d,%d\n", road_x1, road_x2);
    // for (auto i = 0; i < road_x2; i++) { printf("%d(%d) ", stack_x2[i].x,
    // stack_x2[i].y); } printf("\n");
